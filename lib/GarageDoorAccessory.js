...
class GarageDoorAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        if (this._isKogan()) {
            ...
            this.statelessAction = this.device.context.statelessAction;
            if (this.statelessAction) {
                this._debugLog(
                    'acting as switch button');
            }
            this.hasStopFeature = this.device.context.hasStopFeature;
            this.openCloseDuration = this.device.context.openCloseDuration;
        }
        ...
        const characteristicCurrentDoorState = this.characteristicCurrentDoorState;
    ...
    setTargetDoorState(value, callback) {
        ...
        } else if (this.statelessAction) {
            this._debugLog('Setting action to TRUE, acting as button switch');
            newValue = true;
        } else {
        ...
        if (this.hasStopFeature && value === this.targetOpen) {
            if (this.startedToCloseAt) {
                const timeDiff = this.startedToCloseAt && DateTime.now().diff(this.startedToCloseAt, 'seconds').seconds;
                this._debugLog('Started Close At Diff: ', timeDiff);
                if (timeDiff < this.openCloseDuration) {
                    this._debugLog('Open button was pressed while the door was still closing');
                    // Open button was pressed while the door was still closing
                    // this.characteristicCurrentDoorState.updateValue(this.targetOpen);
                    delete this.startedToCloseAt;
                }
            } else {
                this.startedToOpenAt =  DateTime.now();
            }
        } else if (this.hasStopFeature && value === this.targetClosed) {
            if (this.startedToOpenAt) {
                const timeDiff = DateTime.now().diff(this.startedToOpenAt, 'seconds').seconds;
                this._debugLog('Started Open At Diff: ', timeDiff);
                if (timeDiff < this.openCloseDuration) {
                    this._debugLog('Close button was pressed while the door was still opening');
                    this.characteristicCurrentDoorState.updateValue(this.targetOpen);
                    delete this.startedToOpenAt;
                }
            } else {
                this.startedToCloseAt =  DateTime.now();
            }
        }
    }
